...
bool erBST(Node *node)
{
  ...
  int self = node->ID;
  int maxL = finnMax(node->left);
  int minR = finnMin(node->right);

  if ((node->left && maxL > self) || (node->right && minR < self))
    return false;
...
int finnMax(Node *node)
{
  if (!node)
    return MIN;
  int self = node->ID;
  int l = finnMax(node->left);
  int r = finnMax(node->right);
  if (l > self)
    self = l;
  if (r > self)
    self = r;
...
int finnMin(Node *node)
{
  if (!node)
    return MAX;
  int self = node->ID;
  int l = finnMin(node->left);
  int r = finnMin(node->right);
  if (l < self)
    self = l;
  if (r < self)
    self = r;
